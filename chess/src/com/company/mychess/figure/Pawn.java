...
public class Pawn extends Figure implements Move {
    ...
    public void move(int x, int y) throws Exception {
        if (this.figureType == FigureType.BLACK) {//могут двигаться только вниз
            if ((this.y - y ==2&& this.isInitPosition!=false)||(this.y-y==1)) {// вроде как это все условия для движения пешки
                this.x = x;
                this.y = y;
            ...
            if ((this.y - y ==-2&&this.isInitPosition!=false)||(this.y-y==-1)) {// здесь тоже
                this.x = x;
                this.y = y;
